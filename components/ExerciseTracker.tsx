...
import { Plus, Trash2, Camera, X, CalendarDays, Weight, NotebookText, BarChart4, Repeat, PlusCircle, MinusCircle, History, Save, Check, ChevronDown, MapPin, ChevronLeft, ChevronRight, Clock, Flame, Zap, Gauge, TrendingUp, ChevronUp } from 'lucide-react';
...
const ExerciseLogModal: React.FC<ExerciseLogModalProps> = ({ isOpen, onClose, onSave, dayName, initialData }) => {
                        ...
                        {isNadaTab ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="any" inputMode="decimal" value={formData.series} onChange={(e) => handleInputChange('series', e.target.value)} placeholder="Km/h" className={inputClasses} />
                                </div>
                                <div>
                                    ...
                                    <div className="relative">
                                        <input type="number" step="any" inputMode="decimal" value={formData.reps} onChange={(e) => handleInputChange('reps', e.target.value)} placeholder={formData.distanceUnit === 'KM' ? 'Kilómetros' : 'Metros'} className={`${inputClasses} pr-16`} />
                                ...
                                <div>
                                    ...
                                    <input type="number" inputMode="numeric" value={formData.calorias} onChange={(e) => handleInputChange('calorias', e.target.value)} placeholder="Kcal" className={inputClasses} />
                                </div>
                                <div className="sm:col-span-2">
                                    ...
                                    <input type="number" step="any" inputMode="decimal" value={formData.kilos} onChange={(e) => handleInputChange('kilos', e.target.value)} placeholder="%" className={inputClasses} />
                                </div>
                            </div>
                        ) : (
        ...
        <div className="space-y-4 pb-24">
            ...
            {dayLogs.length === 0 ? (
                ...
                dayLogs.map((log, index) => {
                    ...
                    const hasMetrics = hasSeries || hasReps || hasKilos || (isNadaTab && (hasTiempo || hasCalorias));
                    const hasMedia = log.media && log.media.length > 0;
                    const hasNotes = log.notes && log.notes.trim() !== '';
                    const hasAnyDetails = hasMetrics || hasMedia || hasNotes;
                                     ...
                                     <div className="px-3 sm:px-4 pb-3 sm:pb-4">
                                        {(hasAnyDetails) && (
                                            ...
                                            >
                                                        ...
                                                        <div className="flex justify-between items-start gap-4">
                                                            <div className="min-w-0 flex-grow flex justify-center items-center">
                                                                <div className="flex justify-center flex-wrap gap-x-4 gap-y-3 text-sm">
                                                                    {isNadaTab ? (
                                                                        <>
                                                                            {hasTiempo && <div className="flex items-center gap-2"><Clock className="w-5 h-5 text-cyan-400 flex-shrink-0"/><div><p className="text-xs text-gray-400">Tiempo</p><p className="font-semibold text-white">{log.tiempo} Min</p></div></div>}
                                                                            {hasSeries && <div className="flex items-center gap-2"><Zap className="w-5 h-5 text-cyan-400 flex-shrink-0" /><div><p className="text-xs text-gray-400">Velocidad</p><p className="font-semibold text-white">{log.series} Km/h</p></div></div>}
                                                                            {hasReps && <div className="flex items-center gap-2"><Gauge className="w-5 h-5 text-cyan-400 flex-shrink-0" /><div><p className="text-xs text-gray-400">Distancia</p><p className="font-semibold text-white">{log.reps}{log.distanceUnit ? ` ${log.distanceUnit === 'KM' ? 'Kilómetros' : 'Metros'}` : ''}</p></div></div>}
                                                                            {hasCalorias && <div className="flex items-center gap-2"><Flame className="w-5 h-5 text-cyan-400 flex-shrink-0"/><div><p className="text-xs text-gray-400">Calorías</p><p className="font-semibold text-white">{log.calorias} Kcal</p></div></div>}
                                                                            {hasKilos && <div className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-cyan-400 flex-shrink-0" /><div><p className="text-xs text-gray-400">Inclinación</p><p className="font-semibold text-white">{log.kilos} %</p></div></div>}
                                                                        </>
                                                                    ) : (
                                                                        <>
                                                                            ...
                                                                            {hasKilos && <div className="flex items-center gap-2"><Weight className="w-5 h-5 text-cyan-400 flex-shrink-0" /><div><p className="text-xs text-gray-400">Kilos</p><p className="font-semibold text-white">{log.kilos} kgs</p></div></div>}
                                                                        </>
                                                                    )}
                                                                </div>
                                                            </div>
                                                ...
                                                {hasNotes && (
                                                    <div className={`${(hasMetrics || hasMedia) ? 'mt-3 pt-3 border-t' : 'pt-4 border-t'} border-gray-700/50`}>
